...
> This is a **launcher** bundle, not a frozen/self-contained one — it
> depends on this project folder + its `.venv` staying put. That's ideal
> for personal use on one Mac. To ship it to *other* machines you'd want a
> frozen build (py2app), which additionally requires making each module's
> data-file paths "frozen-aware" (they currently resolve relative to
> `__file__`).
